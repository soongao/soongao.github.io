...
export interface ToolRenderResultOptions {
	/** 当前工具结果是否处于展开视图。折叠态通常只展示摘要或少量行。 */
	expanded: boolean;
	/** 当前结果是否只是执行中的部分更新，而不是最终结果。 */
	isPartial: boolean;
}
 ...
 * 传给工具渲染器的上下文。
 *
 * 工具渲染器会被反复调用：参数流式更新、执行开始、部分结果、最终结果、展开/折叠切换都会触发重绘。
 * 因此这里提供 lastComponent 和 state，让工具可以复用已有 TUI 组件，避免每次渲染都重建复杂 UI。
 */
export interface ToolRenderContext<TState = any, TArgs = any> {
	/** 当前 toolCall 参数。同一次工具调用的 call/result 渲染共享这份参数。 */
	args: TArgs;
	/** 当前工具执行的唯一 id。同一次工具调用中保持稳定。 */
	toolCallId: string;
	/** 只让当前工具执行组件失效并请求重绘。 */
	invalidate: () => void;
	/** 这个渲染槽位上一次返回的组件；渲染器可复用它来降低重绘成本。 */
	lastComponent: Component | undefined;
	/** 当前工具行的共享渲染状态，由 tool-execution.ts 初始化并在多次渲染间保留。 */
	state: TState;
	/** 当前工具执行所在的工作目录。 */
	cwd: string;
	/** 工具是否已经开始执行。参数尚未完整流完时可能为 false。 */
...
export interface ExtensionAPI {
	 ...
	 * 注册一个可被 LLM 调用的扩展工具。
	 *
	 * 注册后工具先进入扩展 runtime 的工具表；AgentSession 刷新工具注册表时会把它与内置工具、
	 * SDK customTools 合并，并包装成 AgentTool 暴露给 agent-core。
	 */
...
export interface RegisteredTool {
	...
	/** 工具来源信息，用于 getAllTools、调试、导出和 UI 展示。 */
	sourceInfo: SourceInfo;
}
