...
阅读时可以把很多 UUID 类型都当作字符串看。比如 `pgtype.UUID`、`string`、前端 route id 都是“某个对象的 ID”，只是在不同层被不同库包装。
...
这段 SQL 的作用是：在并发 daemon/多任务环境里，用数据库锁原子地把一个可执行任务从 `queued` 变成 `dispatched`。
...
| `agent_task_queue` | daemon 执行队列 | 后端主要状态机 |
...
前端和 CLI 在请求里携带 workspace 标识。租户校验在后端 middleware 和 SQL 查询里完成。
...
Project resource 表示“项目相关资源指针”。例如：
...
daemon 看到后会把 repo 放进允许列表，并把资源信息写入执行环境。checkout 通常发生在 agent 运行中调用 `multica repo checkout <url>` 时：CLI 请求本机 daemon，daemon 再从 bare repo cache 创建或更新 task workdir 下的 Git worktree。
