...
也就是说，真正传给 `agent.Backend.Execute(ctx, prompt, opts)` 的 `prompt` 只来自这里。
...
本质：这是告诉 agent“你现在被分配了一个 issue，先用 CLI 读 issue，再干活”。真正的 issue title / description / comments 不直接塞进这段 prompt，而是要求 agent 通过 `multica issue get` 和 `multica issue comment list` 拉取。
...
quick-create 是最强 prompt 工程的一类，因为它要把用户一句话变成结构化 issue。
...
- 去掉“创建 issue / 分配给 X / 让 @X 处理”这类路由包装，但保留真正的任务内容。
...
本质：quick-create 的 prompt 是“自然语言到 issue create 命令”的编译器说明。
...
本质：run_only 是“无 issue 的一次 agent run”。如果需要产生长期可见结果，必须由 autopilot description 自己要求写到某个地方，否则最终输出只作为 run result 捕获。
...
本质：slim brief 不改变规则，只是减少无关 section，降低上下文体积。

## 三、`.agent_context/issue_context.md`

源码入口：`server/internal/daemon/execenv/context.go`

`writeContextFiles` 会写 sidecar 文件，最关键的是：
...
本质：这是 squad 模式最核心的 prompt 工程。它把 leader 固定成“协调者”，并用 mention link 把任务送给 teammate。
...
所以 `skill` / `skill_file` 存的是内容，不只是“记录一个本地文件路径”。真正的 `SKILL.md`、脚本、模板文件是在 task 准备阶段物化出来的。
...
本质：这是“创建某种专长 agent 时的默认 persona / behavior prompt”。
...
| card | title zh | description/prompt 本质 |
| --- | --- | --- |
