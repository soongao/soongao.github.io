...
本文基于 `apps/docs/content/docs/*.zh.mdx`，对应线上文档站 `https://multica.ai/docs/zh`。这里整理文档里的产品语义和源码入口，方便后续读代码时定位对象边界。
...
- AI 编程工具负责执行具体工作：Claude Code、Codex、Cursor、OpenCode、Kimi、Kiro CLI 等。

主要安全边界是：智能体不在 Multica server 上运行。API key、代码目录、本地工具登录态都留在运行 daemon 的机器上。
...
重要边界：server 只校验 payload 形状和路径是否像绝对路径，目录存在、读写权限、黑名单路径校验在 daemon 执行前由 daemon 完成。
...
把 issue 分配给 squad 时，入队的是 leader agent 的 task。leader 看到小队花名册和操作协议，然后在评论里用标准 mention markdown 把工作派给某个成员：
